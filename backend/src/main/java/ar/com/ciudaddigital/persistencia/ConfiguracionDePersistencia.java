 ...
 * revés.
 *
 * <p>El reparto es por paquete: las entidades bajo {@code tenants} van a
 * la base de control, y las que están bajo {@code municipio} a la base del
 * tenant en curso.
 */
...
class ConfiguracionDePersistencia {
    ...
    /** Entidades que viven en la base propia de cada municipio. */
    ...
    @Configuration(proxyBeanMethods = false)
    @EnableJpaRepositories(
            basePackages = PAQUETE_MUNICIPIO,
            entityManagerFactoryRef = "tenantEntityManagerFactory",
            transactionManagerRef = "tenantTransactionManager")
    static class RepositoriosDeMunicipio {
    }
    ...
    LocalContainerEntityManagerFactoryBean tenantEntityManagerFactory(DataSourceDeTenants tenantDataSource) {
        ...
        emf.setPackagesToScan(PAQUETE_MUNICIPIO);
